...
import os, time, gin, tensorflow as tf

from absl import app, flags, logging

# from dqn_agent import dqn_agent
from dqn_agent import q_network
...
from tf_agents.policies import py_tf_policy, random_py_policy, random_tf_policy
from tf_agents.replay_buffers import tf_uniform_replay_buffer
from tf_agents.utils import common

flags.DEFINE_string("root_dir", os.getenv("TEST_UNDECLARED_OUTPUTS_DIR"), "root dir for writing summaries")
flags.DEFINE_integer("num_iterations", 100000, "total num of iterations")
FLAGS = flags.FLAGS
...
def train_eval(root_dir,
    ...
    with tf.compat.v2.summary.record_if(lambda: tf.math.equal(global_step % summary_interval, 0)):
        ...
        tf_agent = agent_class(tf_env.time_step_spec(),
                               tf_env.action_spec(),
                               q_network=q_net,
        ...
        initial_collect_policy = random_tf_policy.RandomTFPolicy(tf_env.time_step_spec(),
                                                                 tf_env.action_spec())
        initial_collect_op = dynamic_step_driver.DynamicStepDriver(tf_env,
        ...
        with tf.compat.v1.Session() as sess:
            ...
            steps_per_second_summary = tf.compat.v2.summary.scalar(name="global_step_per_sec",
            ...
            for _ in range(num_iterations):
                ...
                if global_step_val % log_interval == 0:
                    logging.info("step = %d, loss = %f", global_step_val, loss_info_value.loss)
                    step_per_sec = ((global_step_val - timed_at_step) / (collect_time + train_time))
                    sess.run(steps_per_second_summary, feed_dict={steps_per_second_ph: step_per_sec})
                    logging.info("%.3f steps/sec", step_per_sec)
                    logging.info("%s", "collect_time = {}, train_time = {}".format(collect_time, train_time))
